...
export class GeneticAlgorithm<T> {
  ...
  private async createIndividual(length: number): Promise<Individual<T>> {
    ...
    if (!this.operators.createGene) {
      throw new Error(
        "Either createIndividual or createGene operator must be provided",
      );
    }
    ...
    while (generation < this.config.generationLimit) {
      ...
      if (generation % 10 === 0) {
        ...
        const bestFitness = Math.max(
          ...currentPopulation.map((ind) => ind.fitness),
        );
  ...
  public async evolve(
    ...
    const initialPopulation =
      await this.createInitialPopulation(individualLength);
