import { GeneticAlgorithm } from "../genetic-core";
...
export class SudokuSolver {
  ...
  private validateBoard(board: SudokuBoard): void {
    ...
    for (let i = 0; i < 9; i++) {
      if (!Array.isArray(board[i]) || board[i].length !== 9) {
        throw new SudokuError(
          `Invalid row ${i}: must contain exactly 9 numbers`,
        );
      }
      ...
      for (let j = 0; j < 9; j++) {
        ...
        if (!Number.isInteger(cell) || cell < 0 || cell > 9) {
          throw new SudokuError(
            `Invalid value at position [${i},${j}]: must be a number between 0 and 9`,
          );
        }
      }
    }
    ...
    if (!isValidBoard(board)) {
      throw new SudokuError(
        "Invalid board: contains duplicate numbers in row, column, or 3x3 box",
      );
    }
  }
  ...
  async solve(
    ...
    try {
      ...
      const { population: finalPopulation, generations } =
        await geneticAlgorithm.evolve(9, onGenerationComplete);
